...
export default function CustomerForm({ onSubmit, loading }: CustomerFormProps) {
  ...
  const validate = (): boolean => {
    ...
    if (!form.customerPhone.trim() || form.customerPhone.replace(/\D/g, '').length < 8)
      e.customerPhone = 'Valid phone number required';
    setErrors(e);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="bg-brand-50 rounded-xl p-4 text-sm text-brand-800 border border-brand-100">
        <p className="font-semibold mb-0.5">📋 Booking confirmation</p>
        <p className="text-xs text-brand-700">
          Your booking will be confirmed via WhatsApp. No upfront payment required. Driver details will be sent 24 hours before pickup.
        </p>
      </div>
