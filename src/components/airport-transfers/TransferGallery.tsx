...
export default function TransferGallery({ images, title }: TransferGalleryProps) {
  ...
  return (
    <>
      {/* ── Gallery Grid ─────────────────────────────────────────────────────── */}
      <section aria-label="Transfer photos" className="bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-0">
          <div className="relative grid grid-cols-1 sm:grid-cols-3 gap-2 h-[260px] sm:h-[420px] rounded-2xl overflow-hidden">
            ...
            <div className="hidden sm:flex flex-col gap-2">
              ...
              >
                {thumb1 ? (
                  <>
                    <Image src={thumb1} alt={`${title} — photo 2`} fill
              ...
              >
                {thumb2 ? (
                  <>
                    <Image src={thumb2} alt={`${title} — photo 3`} fill
                      className="object-cover transition-transform duration-500 group-hover:scale-[1.04]"
                      sizes="33vw" unoptimized />
                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/15 transition-colors" />
                    {safeImages.length > 3 && (
        ...
        >
          ...
          <div className="relative flex-1 flex items-center justify-center px-14 min-h-0">
            <Image
              ...
              fill className="object-contain" sizes="100vw" priority unoptimized
            />
            {safeImages.length > 1 && (
              <>
              ...
              {safeImages.map((img, i) => (
                <button key={i} onClick={() => setCurrentIndex(i)}
                  className={`relative shrink-0 w-14 h-10 rounded-lg overflow-hidden transition-all ${
                    i === currentIndex ? 'ring-2 ring-white opacity-100 scale-105' : 'opacity-50 hover:opacity-80'
                  }`}
